...
runners.forEach(function(element) {
    return fullNames.push(element.first_name), fullNames.push(element.last_name);
});
...
let firstNamesAllCaps = runners.map(function(element) {
    return (element.first_name.charAt(0).toUpperCase() + element.first_name.slice(1))
});
// runners.map(function(element) {
//     return firstNamesAllCaps.push()
// })
...
let allShirtSizes = [];
runners.forEach(function(element) {
    return allShirtSizes.push(element.shirt_size);
})
console.log(allShirtSizes)
    // Problem 2
    // Return the chairman's info based off of last name
let lastNames = runners.filter(function(element) {
    return element.last_name === "Seiler";
})
...
let avgDonation = runners.reduce(function(accumulator, num) {
    return (accumulator + num.donation);
}, 0)

let avg = (avgDonation / runners.length);
